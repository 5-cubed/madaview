...
`.context/intents/sidebar-folding.md` (not yet implemented). This spec adds
the fold/unfold toggle's state and its persistence so the fold choice
survives a page reload, and confirms the remaining implementation-level
details (storage key, fallback behavior, cross-tab scope, transition
timing) needed to make it testable.

`react-icons` (the `vsc` subset, for the toggle icon) is assumed already
added per the separate draft RDR `20260716-213757-react-icons.md` — that
dependency work is out of scope here.
...
## Out of Scope
- Adding the `react-icons` dependency — covered by draft RDR
  `20260716-213757-react-icons.md`.
